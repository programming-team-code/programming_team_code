//! @file
#pragma once
//! @see https://codeforces.com/blog/entry/104997
//! https://codeforces.com/blog/entry/120446
//!
//! only handle paths with >=1 edge in each edge-set
//!     (it is guaranteed the edge-sets are
//!     non-empty i.e. 0 < split < sz(adj[cent]))
//! don't handle cent<->v paths as these will be
//! handled in some smaller decomposition
//!     (except for single-edge paths, so handle
//!     these separately)
//!
//! @code{.cpp}
//!     edge_cd(adj, [&](const vector<vi>& adj, int
//!     cent, int split) {
//!         // subtrees of prefix [0, split) of
//!         adj[cent] are the first edge-set
//!         // subtrees of suffix [split,
//!         sz(adj[cent])) of adj[cent] are the
//!         second edge-set
//!     });
//! @endcode
template <class F> struct edge_cd {
  ...
  //! recursion stack for `dfs` is O(log1.5 n);
  //! recursion stack for `find_cent` is O(n)
