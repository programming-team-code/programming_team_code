//! @file
#pragma once
//! helper for edge CD: given tree, update node's
//! number, find sum of neighbors' numbers
template <class T> struct sum_adj {
  int n;
  vector<T> sum, sum_ch;
  vi p;
  //! @param adj undirected, unrooted tree
  //! @param a_sum a_sum[v] = initial number for
  //! node v
  //! @time O(n)
  //! @space various O(n) vectors are allocated;
