...
//! @endcode
struct subtree_iso {
  int num_distinct_subtrees;  //!< number of classes
                              //!< (by iso.) of
                              //!< subtrees
  ...
  subtree_iso(const vector<vi>& adj)
      : iso_id(sz(adj), -1) {
    map<vi, int> hashes;
    auto dfs = [&](
      ...
      for (int u : adj[v])
        if (u != p)
          ch_ids.push_back(self(self, u, v));
      sort(all(ch_ids));
      return iso_id[v] =
               hashes
                 .try_emplace(ch_ids, sz(hashes))
                 .first->second;
    };
    rep(i, 0, sz(adj)) if (iso_id[i] == -1)
      dfs(dfs, i, i);
