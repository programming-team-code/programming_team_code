...
//! compute the minimal subtree that contains all
//! the nodes by adding all (at most |S|-1) pairwise
//! LCA's and compressing edges.
//!
//! @code{.cpp}
//!     auto [par, orig_node] =
...
//! @returns vectors `par`, `orig_node`, for node i
//! in compressed tree:
//!          par[i] = parent of i in compressed
//!          tree; par[0]
//!          == -1; par[i] < i orig_node[i] =
//!          corresponding node in big tree
