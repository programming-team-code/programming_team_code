...
//! = number of paths with k edges where node `i` is
//! on the path. 0-based nodes.
//! @time O(n log n)
//! @space this function allocates/returns various
//! vectors which are all O(n)
vector<ll> count_paths_per_node(
  const vector<vi>& adj, int k) {
  ...
  centroid(
    adj, [&](const vector<vi>& cd_adj, int cent) {
      ...
      auto dfs_child = [&](int child) -> ll {
        ...
        for (int i = 1; i < sz(cur_d) && cur_d[i];
             i++)
      ...
      pre_d = cur_d = {0};
      for_each(rbegin(cd_adj[cent]),
        rend(cd_adj[cent]), dfs_child);
    });
  return num_paths;
}
