...
struct wavelet_tree_updates {
  ...
  //!     begin(sorted); wavelet_tree_updates wtu(a,
  ...
  void build(vector<pair<int, bool>>& cpy, int le,
    int ri, int tl, int tr, int v) {
  ...
  //! @param k must satisfy 1 <= k <= # active
  //! indexes in [le, ri)
  //! @returns kth smallest active number in range.
  //!     - kth_smallest(le,ri,1) returns the
  //!     smallest active number
  //!     -
