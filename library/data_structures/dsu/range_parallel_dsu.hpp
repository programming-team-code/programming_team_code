...
//! + len) and [l2, l2 + len) such that edges of the form (l1
//! + i, l2 + i) are joined. for all `i` in [0, len).
//!
...
struct range_parallel_dsu {
  ...
  //! joins the ranges [l1, l1 + len) and [l2, l2 + len) such
  //! that edges of the form (l1 + i, l2 + i) are joined. for
  //! all `i` in [0, len). The function `f` is called for
  //! each connected component of the resulting graph which
  //! will change in this update, with the first argument
  //! being the representative of the component and the
  //! second argument being the parent of the component which
  //! is being joined.
  //!
