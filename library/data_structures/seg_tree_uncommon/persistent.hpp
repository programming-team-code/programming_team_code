...
struct PST {
  ...
  int root_l, root_r;
  vi roots;  //!< tree[roots[i]] = root node at
             //!< version i
  deque<node> tree;
  //! @param a_root_l,a_root_r defines range
  //! [root_l, root_l) of root node, can be negative
  //! @time O(1)
  //! @space O(1)
  ...
  //! Initially there is a dummy version.
  //! @time O(log(root_r - root_l))
  //! @space O(log(root_r - root_l)) new nodes are
  //! pushed back onto `tree`; (and for recursion
  //! stack)
  void update(int idx, ll change, int version) {
    roots.push_back(update_impl(
      idx, change, root_l, root_r, roots[version]));
  }
  int update_impl(
    int idx, ll change, int tl, int tr, int v) {
    if (tr - tl == 1) {
    ...
    tree.emplace_back(
      tree[lch].sum + tree[rch].sum, lch, rch);
    return sz(tree) - 1;
  }
  //! @param le, ri defines range [le, ri)
  //! @param version which version to query
  //! @returns a[le] + a[le + 1] + ... + a[ri - 1]
  //! during the `version`-th version
  //! @time O(log(root_r - root_l))
  //! @space O(log(root_r - root_l)) for recursion
  //! stack, no new nodes are allocated
  ll query(int le, int ri, int version) {
    return query_impl(
      le, ri, root_l, root_r, roots[version]);
  }
  ll query_impl(
    int le, int ri, int tl, int tr, int v) {
