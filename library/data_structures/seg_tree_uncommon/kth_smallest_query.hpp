...
struct kth_smallest {
  PST pst;
  //! @param a,minv,maxv must satisfy: minv <= a[i]
  //! < maxv
  //! @time O(n log(maxv - minv))
  //! @space O(n log(maxv - minv)) nodes are pushed
  //! back onto PST::tree
  ...
  //! @space O(log(maxv - minv)) for recursion
  ...
  int query_impl(
    int k, int tl, int tr, int vl, int vr) {
    if (tr - tl == 1) return tl;
    int tm = tl + (tr - tl) / 2;
    int left_count =
      pst.tree[pst.tree[vr].lch].sum -
