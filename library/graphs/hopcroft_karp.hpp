...
struct hopcroft_karp {
  int size_of_matching =
    0;  //!< # of edges in max matching (which =
        //!< size of min vertex cover by König's
        //!< theorem)
  //! edge node_left <=> l_to_r[node_left] in
  //! matching iff l_to_r[node_left] != -1 ditto
  //! r_to_l[node_right] <=> node_right
  //! @{
  vi l_to_r, r_to_l;
  //! @}
  //! mvc_l[node_left] = 1 iff node_left is in the
  //! min vertex cover; ditto mvc_r[node_right]
  //! mvc_l[node_left] = 0 iff node_left is in the
  //! max independent set
  //! @{
  ...
  //! @space this allocates member vectors which are
  //! O(lsz + rsz)
  hopcroft_karp(const vector<vi>& adj, int rsz)
    ...
    while (1) {
      queue<int> q;
      vector level(lsz, -1);
      rep(i, 0, lsz) if (l_to_r[i] == -1) level[i] =
        0,
                                          q.push(i);
      bool found = 0;
      mvc_l.assign(lsz, 1);
