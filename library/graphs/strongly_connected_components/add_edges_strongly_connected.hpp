...
//!
//! Add minimum extra directed edges to directed
//! graph to make it strongly connected. If the
//! directed graph is not initially strongly
//! connected, then the edge list's size is max(#
//! zero indegree SCCs, # zero outdegree SCCs)
//!
//! @code{.cpp}
...
//!     num_sccs, scc_id);
//! @endcode
//! @param adj,num_sccs,scc_id directed graph and
//! its SCCs
//! @returns directed edge list: edges[i][0] ->
//! edges[i][1]
//! @time O(n + m)
//! @space An O(n) edge list is allocated and
//! returned, but multiple O(n + m) vectors are
...
vector<pii> extra_edges(const vector<vi>& adj,
  ...
  rep(i, 0,
    num_sccs) if (empty(scc_adj[i]) && !vis[i]) {
    if (!empty(in_unused)) {
