...
template<class T> struct sa_query {
  ...
  //! @space O(nlogn + max_val) used during constructor; but
  //! member variables are only O(nlogn)
  sa_query(const T& a_s, int max_val):
    sa_query(a_s, get_sa(a_s, max_val)) {}
  //! @param i1,i2 defines suffixes [i1, n), [i2, n)
  //!      - if one of the suffixes is the entire string (i1
  //!      == 0 || i2 == 0) then
  //!        it's the same as z algorithm
  //!      - doesn't work when i1,i2 == n
  //! @returns max integer k such that s.substr(i1, k) ==
  //! s.substr(i2, k)
  //! @time O(1)
  //! @space O(1)
