...
//!     given s_ri, t_le, t_ri; calculate
//!     size(LCS(s[0, s_ri), t[t_le, t_ri)))
//!
//! @param s,t strings/arrays
//! @param queries queries[i] = {s_ri, t_le, t_ri}
//! @returns res[i] = size(LCS(s.substr(0,
//! queries[i][0]), t.substr(queries[i][1],
//! queries[i][2] - queries[i][1])))
//! @time O(n*m*log(m) + q*log(m) + q*log(q))
//! @space this allocates/returns a O(q) vector, and
//! various vectors are allocated temporarily which
//! are O(n + m + q)
template <class T> vi lcs_queries(const T& s,
  const T& t,
  ...
  rep(i, 0, n) {
    ...
    rep(t_le, 0, m) {
      while (j < sz(qs[i]) && qs[i][j][0] == t_le)
        res[qs[i][j][2]] =
          bit.query(t_le, qs[i][j][1]),
        j++;
      if (dp_inv[t_le] != -1)
        bit.update(dp_inv[t_le], 1);
    }
  }
  return res;
}
