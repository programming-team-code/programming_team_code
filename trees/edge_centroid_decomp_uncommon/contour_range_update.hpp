...
struct sum_adj {
  ...
  sum_adj(const vector<vector<int>>& adj,
    const vector<int64_t>& a_sum):
    n(ssize(a_sum)), sum(a_sum), sum_ch(n), p(n, -1) {
...
struct contour_range_update {
  ...
  contour_range_update(const vector<vector<int>>& adj,
    const vector<int64_t>& a_a):
    n(ssize(a_a)), a(a_a), sum_a(adj, vector<int64_t>(n)),
    info(n) {
    edge_cd(adj,
      [&](const vector<vector<int>>& cd_adj, int cent,
        ...
        auto dfs = [&](auto&& self, int v, int p, int d,
          ...
          info[v].push_back({ssize(bits), d, side});
          for (int u : cd_adj[v])
