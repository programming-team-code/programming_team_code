...
struct LCA {
  ...
  LCA(const auto& g): n(ssize(g)), tin(n), siz(n, 1), d(n), p(n) {
    vector<int> order;
    // pass-self, not deducing this: gcc bug 118291
    auto dfs = [&](auto&& dfs, int u) -> void {
      tin[u] = ssize(order), order.push_back(u);
      for (int v : g[u])
        if (v != p[u]) d[v] = d[p[v] = u] + 1, dfs(dfs, v), siz[u] += siz[v];
    };
    dfs(dfs, 0);
  ...
  bool in_subtree(int u, int v) { return tin[u] <= tin[v] && tin[v] < tin[u] + siz[u]; }
  //! returns 1 if w is on path from u to v
  bool on_path(int u, int v, int w) { return (in_subtree(w, u) || in_subtree(w, v)) && in_subtree(lca(u, v), w); }
  //! https://codeforces.com/blog/entry/71567?#comment-559285
  //! returns the node
  //!   vector<int>({u,p[u],..,lca(u,v),..,p[v],v})[1]
  //! @time O(1)
  //! @space O(1)
  ...
  array<vector<int>, 2> compress_tree(vector<int> subset) {
    ...
    for (int i = 1; i < len; i++) subset.push_back(lca(subset[i - 1], subset[i]));
    ranges::sort(subset, {}, proj);
