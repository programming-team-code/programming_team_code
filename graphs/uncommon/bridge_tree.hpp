...
auto bridges(const auto& g, int m) {
  int n = ssize(g), num_ccs = 0, q = 0, s = 0;
  vector<int> br_id(n, -1), is_br(m), tin(n), st(n);
  auto dfs = [&](auto dfs, int u, int p) -> int {
    int low = tin[u] = ++q;
    st[s++] = u;
    for (auto [v, e] : g[u])
      if (e != p && br_id[v] < 0) low = min(low, tin[v] ?: dfs(dfs, v, e));
    if (tin[u] == low) {
      if (p != -1) is_br[p] = 1;
...
//! @time O(n + m)
//! @space O(n)
auto bridge_tree(const auto& g, int num_ccs, const vector<int>& br_id, const vector<int>& is_br) {
  vector<vector<int>> tree(num_ccs);
  for (int i = 0; i < ssize(g); i++)
    for (auto [u, e_id] : g[i])
      if (is_br[e_id]) tree[br_id[i]].push_back(br_id[u]);
  return tree;
}
