...
vector<pair<int, int>> extra_edges(const auto& g, int num_sccs, const vector<int>& scc_id) {
  if (num_sccs == 1) return {};
  int n = ssize(g);
  vector<vector<int>> scc_g(num_sccs);
  ...
  for (int i = 1; i < ssize(edges); i++) swap(edges[i].first, edges[i - 1].first);
  for (int i = 0; i < num_sccs; i++)
    if (empty(scc_g[i]) && !vis[i]) {
      if (!empty(in_unused)) {
