...
auto scc(const auto& g) {
  int n = ssize(g), num_sccs = 0, q = 0, s = 0;
  vector<int> scc_id(n, -1), tin(n), st(n);
  auto dfs = [&](auto&& dfs, int u) -> int {
    int low = tin[u] = ++q;
    st[s++] = u;
    for (int v : g[u])
      if (scc_id[v] < 0) low = min(low, tin[v] ?: dfs(dfs, v));
    if (tin[u] == low) {
...
//! @time O((n + m) log m)
//! @space O(n + m)
vector<int> offline_incremental_scc(vector<array<int, 2>> eds, int n) {
  int m = ssize(eds);
  vector<int> ids(n, -1), joins(m, m), idx(m), vs(n), scc_id;
  iota(begin(idx), end(idx), 0);
  vector<vector<int>> g;
  auto dnc = [&](auto&& dnc, auto el, auto er, int tl, int tr) {
    g.clear();
    ...
    for (int i = 0; i < ssize(g); i++) ids[vs[i]] = -1;
    scc_id = scc(g).second;
    auto split = partition(el, er, [&](int i) { return scc_id[eds[i][0]] == scc_id[eds[i][1]]; });
