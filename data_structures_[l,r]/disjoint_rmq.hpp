...
template<class T, class F> struct disjoint_rmq {
  ...
  disjoint_rmq(const vector<T>& a, F op): op(op) {
    for (int len = 1, n = ssize(a); len <= n; len *= 2) {
      dp.emplace_back(n);
      for (int l = 0; l < n; l += 2 * len) {
        int mi = min(n, l + len), r = min(n, l + 2 * len);
        partial_sum(rend(a) - mi, rend(a) - l, rend(dp.back()) - mi, [&](T x, T y) { return op(y, x); });
        partial_sum(begin(a) + mi, begin(a) + r, begin(dp.back()) + mi, op);
      }
    }
  }
  T query(int l, int r) {
