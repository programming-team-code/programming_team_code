...
struct seg_tree {
  ...
  void update(int l, int r, int64_t d) { update(l, r, d, 0, n - 1, 1); }
  ...
  int64_t query(int l, int r) { return query(l, r, 0, n - 1, 1); }
  int64_t query(int l, int r, int tl, int tr, int v) {
    if (r < tl || tr < l) return 0;
    if (l <= tl && tr <= r) return tree[v];
    int tm = split(tl, tr);
    push(tl, tm, tr, v);
    return op(query(l, r, tl, tm, 2 * v), query(l, r, tm + 1, tr, 2 * v + 1));
  }
};
