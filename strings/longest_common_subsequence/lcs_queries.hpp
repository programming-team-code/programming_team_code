...
template<class T> struct lcs_dp {
  T t;
  vector<int> dp;
  lcs_dp(const T& t): t(t), dp(ssize(t)) { iota(begin(dp), end(dp), 0); }
...
//! @space O(n + m + q)
vector<int> lcs_queries(const auto& s, const auto& t, const vector<array<int, 3>>& queries) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    for (int t_le = 0; t_le < m; t_le++) {
      while (j < ssize(qs[i]) && qs[i][j][0] == t_le) res[qs[i][j][2]] = bit.query(t_le, qs[i][j][1]), j++;
