...
vector<int> manacher(const auto& s) {
  ...
  for (int i = 0; i < 2 * n - 1; i++) {
    int r = i <= 2 * (p - man[p]) ? p - max(man[2 * p - i], man[p]) : i / 2;
    man[i] = i - r;
    while (man[i] > 0 && r + 1 < n && s[man[i] - 1] == s[r + 1]) man[i]--, r++, p = i;
  }
  return man;
}
...
struct longest_pal_query {
  ...
  pair<int, int> longest_pal(int l, int r) {
    assert(l < r);
    int pal_len = *ranges::partition_point(views::iota(0, r - l), [&](int x) { return len(rmq.query(2 * l + x, 2 * r - x - 1)) > x; });
    int best_center = rmq.query(2 * l + pal_len - 1, 2 * r - pal_len);
