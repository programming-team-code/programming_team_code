...
auto get_sa(const auto& s, int max_num) {
  ...
  for (int i = 0; i < n; i = max(1, 2 * i)) {
    ...
    ranges::copy_if(sa, begin(sa_inv) + i, [&](int& x) { return (x -= i) >= 0; });
    for (int x : y) freq[x]++;
    partial_sum(begin(freq), end(freq), begin(freq));
    for (int x : sa_inv | views::reverse) sa[--freq[y[x]]] = x;
    max_num = 1, sa_inv[sa[0]] = 0;
    auto prev_inv = [&](int j) { return pair(y[j], i + j < n ? y[i + j] : -1); };
    for (int j = 1; j < n; j++) {
...
template<class T, class F> struct RMQ {
  ...
  RMQ(const vector<T>& a, F op): dp(1, a), op(op) {
    for (int i = 0; (2 << i) <= ssize(a); i++) {
      ...
      ranges::transform(dp[i], dp[i] | views::drop(1 << i), begin(dp[i + 1]), op);
    }
  }
  T query(int l, int r) {
    assert(l < r);
    int lg = bit_width((r - l) / 2u);
...
template<class T> struct sa_query {
  ...
  sa_query(const T& s, const vector<int>& sa, const vector<int>& sa_inv, const vector<int>& lcp): n(ssize(s)), s(s), sa(sa), sa_inv(sa_inv), lcp(lcp), rmq(lcp, ranges::min) {}
  ...
  //! @time O(1)
  //! @space O(1)
  int cmp_sufs(int l1, int l2) { return sa_inv[l1] - sa_inv[l2]; }
  //! @code
  //!   string s;
  //!   auto [sa, sa_inv, lcp] = get_sa(s, 256);
  //!   sa_query saq(s, sa, sa_inv, lcp);
  ...
  int cmp_substrs(int l1, int r1, int l2, int r2) {
    ...
    return len_lcp(l1, l2) >= min(len1, len2) ? len1 - len2 : cmp_sufs(l1, l2);
  }
  ...
  pair<int, int> find_str(const T& t) {
    auto [l, r] = ranges::equal_range(sa, 0, {}, [&](int i) { return bit_cast<char>(lexicographical_compare_three_way(begin(s) + i, begin(s) + min(i + ssize(t), ssize(s)), begin(t), end(t))); });
  ...
  match find_str_fast(const T& t) {
    ...
    auto cmp = [&](int i, int) -> bool {
      if (int lcp_len = s_l == n ? 0 : len_lcp(s_l, i); lcp_len != s_len) return (lcp_len < s_len) ^ (cmp_sufs(s_l, i) < 0);
      auto [it_s, it_t] = ranges::mismatch(s | views::drop(i + s_len), t | views::drop(s_len));
      s_l = i, s_len = it_t - begin(t);
      return lexicographical_compare(it_s, end(s), it_t, end(t));
    };
    int sa_le = ranges::lower_bound(sa, 0, cmp) - begin(sa), sa_ri = sa_le;
    if (s_len == ssize(t)) sa_ri = ranges::lower_bound(sa | views::drop(sa_le), 0, [&](int i, int) -> bool { return len_lcp(s_l, i) >= ssize(t); }) - begin(sa);
  ...
  pair<int, int> find_substr(int s_l, int s_r) {
    auto cmp = [&](int i, bool flip) -> bool { return flip ^ (len_lcp(i, s_l) < s_r - s_l); };
    int l = ranges::lower_bound(sa | views::take(sa_inv[s_l]), 0, cmp) - begin(sa);
    int r = ranges::lower_bound(sa | views::drop(sa_inv[s_l] + 1), 1, cmp) - begin(sa);
    return {l, r};
  }
  ...
  //! @space O(1)
  match find_substrs_concated(const vector<pair<int, int>>& substrs) {
    ...
    auto cmp = [&](int i, const dt& x) -> bool {
      ...
      if (lcp_len + sum_len > s_r - s_l) s_l = i, s_r = j + lcp_len;
      if (lcp_len < min(n - j, x[1])) return sa_inv[j] - sa_inv[x[0]] < x[2];
      return x[2] ^ (n - j < x[1]);
    };
    for (auto [l, r] : substrs) {
      sa_le = lower_bound(begin(sa) + sa_le, begin(sa) + sa_ri, dt{l, r - l, 0}, cmp) - begin(sa);
      sa_ri = lower_bound(begin(sa) + sa_le, begin(sa) + sa_ri, dt{l, r - l, 1}, cmp) - begin(sa);
      sum_len += r - l;
    }
