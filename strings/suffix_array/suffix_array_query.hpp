...
template<class T> struct sa_query {
  ...
  match find_str_fast(const T& t) {
    ...
    if (s_len == ssize(t)) sa_ri = lower_bound(sa_le + begin(sa), end(sa), 0, [&](int i, int) -> bool { return len_lcp(s_l, i) >= ssize(t); }) - begin(sa);
  ...
  pair<int, int> find_substr(int s_l, int s_r) {
    ...
    int l = lower_bound(begin(sa), begin(sa) + sa_inv[s_l], 0, cmp) - begin(sa);
    int r = lower_bound(sa_inv[s_l] + 1 + begin(sa), end(sa), 1, cmp) - begin(sa);
    return {l, r};
  }
