...
array<vl, 2> get_right_and_top(vl left, vl bottom) {
  ...
  for (vl &cur : res) {
    {
      vl tr(ssize(left));
      for (int i = 0; i < ssize(tr); i++) tr[i] = C(i + ssize(bottom) - 1, i);
      cur = conv(left, tr);
      cur.resize(ssize(left));
    }
    {
      vl tr(ssize(left) + ssize(bottom));
      grow(ssize(tr));
      for (int i = 0; i < ssize(tr); i++) tr[i] = t[i].fact;
      vl dp(ssize(bottom));
      for (int i = 0; i < ssize(dp); i++) dp[i] = bottom[i] * t[ssize(dp) - 1 - i].inv_fact % mod;
      vl tmp = conv(dp, tr);
      for (int i = 0; i < ssize(cur); i++) cur[i] = (cur[i] + tmp[i + ssize(bottom) - 1] * t[i].inv_fact) % mod;
    }
    swap(left, bottom);
  }
  return res;
...
vl divide_and_conquer(vl h, vl bottom) {
  {
    int start = ranges::find_if(h, [](int64_t x) { return x; }) - begin(h);
  ...
  int mid = n / 2;
  auto rect_left = divide_and_conquer(vl(begin(h), begin(h) + mid), vl(begin(bottom), begin(bottom) + mid));
