...
void ntt(vl &a) {
  int n = (int)size(a), L = 31 - __builtin_clz(n);
  static vl rt(2, 1);
...
vl conv(const vl &a, const vl &b) {
  ...
  int s = (int)size(a) + (int)size(b) - 1, B = 32 - __builtin_clz(s), n = 1 << B;
  int inv = modpow(n, mod - 2);
  vl L(a), R(b), out(n);
  L.resize(n), R.resize(n);
  ntt(L), ntt(R);
  for (int i = 0; i < (n); i++) out[-i & (n - 1)] = (int64_t)L[i] * R[i] % mod * inv % mod;
  ntt(out);
...
array<vl, 2> get_right_and_top(vl left, vl bottom) {
  ...
  for (vl &res : ret) {
    {
      vl tr((int)size(left));
      for (int i = 0; i < ((int)size(tr)); i++) tr[i] = C(i + (int)size(bottom) - 1, i);
      res = conv(left, tr);
      res.resize((int)size(left));
    }
    {
      vl tr((int)size(left) + (int)size(bottom));
      grow((int)size(tr));
      for (int i = 0; i < ((int)size(tr)); i++) tr[i] = t[i].fact;
      vl dp((int)size(bottom));
      for (int i = 0; i < ((int)size(dp)); i++) dp[i] = bottom[i] * t[(int)size(dp) - 1 - i].inv_fact % mod;
      vl tmp_res = conv(dp, tr);
      for (int i = 0; i < ((int)size(res)); i++) res[i] = (res[i] + tmp_res[i + (int)size(bottom) - 1] * t[i].inv_fact) % mod;
    }
    swap(left, bottom);
  }
  return ret;
...
vl divide_and_conquer(vl h, vl bottom) {
  {
    int start = find_if(begin(h), end(h), [](int64_t x) { return x; }) - begin(h);
  ...
  auto [rect_right, rect_top] = get_right_and_top(rect_left, vl(mid + begin(bottom), end(bottom)));
  vl h_right(mid + begin(h), end(h));
  for (int64_t &hh : h_right) hh -= h[mid - 1];
  auto upper_right = divide_and_conquer(h_right, rect_top);
  rect_right.insert(end(rect_right), begin(upper_right), end(upper_right));
  return rect_right;
}
