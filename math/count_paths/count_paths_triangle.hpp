...
array<vl, 2> get_right_and_top(vl left, vl bottom) {
  ...
  for (vl &res : ret) {
    {
      vl tr((int)size(left));
      for (int i = 0; i < ((int)size(tr)); i++)
        tr[i] = C(i + (int)size(bottom) - 1, i);
      res = conv(left, tr);
    ...
    }
    {
      ...
      for (int i = 0; i < ((int)size(dp)); i++)
        dp[i] = bottom[i] *
          t[(int)size(dp) - 1 - i].inv_fact % mod;
      vl tmp_res = conv(dp, tr);
      for (int i = 0; i < ((int)size(res)); i++)
        res[i] = (res[i] +
                   tmp_res[i + (int)size(bottom) - 1] *
                     t[i].inv_fact) %
          mod;
    }
    swap(left, bottom);
...
vl divide_and_conquer(vl h, vl bottom) {
  {
    int start = find_if(begin(h), end(h),
                  [](int64_t x) { return x; }) -
      begin(h);
  ...
  auto [rect_right, rect_top] = get_right_and_top(
    rect_left, vl(mid + begin(bottom), end(bottom)));
  vl h_right(mid + begin(h), end(h));
  for (int64_t &hh : h_right) hh -= h[mid - 1];
  auto upper_right = divide_and_conquer(h_right, rect_top);
  rect_right.insert(end(rect_right), begin(upper_right),
    end(upper_right));
  return rect_right;
}
