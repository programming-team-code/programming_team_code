...
//! @space O(1)
pair<int, int> row_reduce(vector<vector<int>>& mat, int cols) {
  int n = ssize(mat), m = ssize(mat[0]), rank = 0;
  int det = 1;
  for (int col = 0; col < cols && rank < n; col++) {
    auto it = find_if(rank + begin(mat), end(mat), [&](auto& v) { return v[col]; });
    if (it == end(mat)) {
      det = 0;
      continue;
    }
    ...
    for (int& num : mat[rank]) num = 1LL * num * a_inv % mod;
    for (int i = 0; i < n; i++)
      if (i != rank && mat[i][col] != 0) {
        int num = mat[i][col];
        for (int j = 0; j < m; j++) mat[i][j] = ((mat[i][j] - 1LL * mat[rank][j] * num) % mod + mod) % mod;
      }
    rank++;
  }
  return {rank, det};
...
struct solve_linear_mod {
  int rank, det;
  vector<int> sol;
  solve_linear_mod(vector<vector<int>>& mat, const vector<int>& rhs) {
    int n = ssize(mat), m = ssize(mat[0]);
    for (int i = 0; i < n; i++) mat[i].push_back(rhs[i]);
    tie(rank, det) = row_reduce(mat, m);
    if (ranges::any_of(mat | views::drop(rank), [](vector<int>& v) { return v.back(); })) return;
    sol.resize(m);
    int j = 0;
    ranges::for_each(mat | views::take(rank), [&](vector<int>& v) {
      while (!v[j]) j++;
      sol[j] = v.back();
    });
  }
