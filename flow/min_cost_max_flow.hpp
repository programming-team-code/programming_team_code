...
struct mcmf {
  ...
  void add_edge(int u, int v, int64_t cap, int64_t cost) {
    edge e1 = {u, v, cap, cost, 0, ssize(g[v])};
    edge e2 = {v, u, 0, -cost, 0, ssize(g[u])};
    g[u].push_back(ssize(e));
    e.push_back(e1);
    g[v].push_back(ssize(e));
    e.push_back(e2);
  }
  array<int64_t, 2> get_flow(int s, int t, int64_t total_flow) {
    ...
    while (flow < total_flow) {
      ...
      for (int u = t; u != s; u = p[u]) {
        int pv = p[u], pr = p_edge[u], r = e[g[pv][pr]].back;
