 ...
 */
vector<int> min_plus_convolution_convex_and_arbitrary(const vector<int>& convex,
                                                      const vector<int>& arbitrary) {
  int n = (int)size(convex);
  int m = (int)size(arbitrary);
  vector<int> res(n + m - 1, INT_MAX);
  auto dnc = [&](auto&& self, int res_le, int res_ri, int arb_le,
                 int arb_ri) -> void {
    if (res_le >= res_ri) return;
    int mid_res = (res_le + res_ri) / 2;
    int op_arb = arb_le;
    for (int i = arb_le; i < (min(mid_res + 1, arb_ri)); i++) {
      int j = mid_res - i;
      if (j >= n) continue;
    ...
    self(self, res_le, mid_res, arb_le, min(arb_ri, op_arb + 1));
