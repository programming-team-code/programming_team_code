...
struct bit_vec {
  ...
  bit_vec(const vector<bool>& a): b(ssize(a) / 64 + 1) {
    ...
    for (int i = 1; i < ssize(b); i++) b[i].second += b[i - 1].second;
  }
  int cnt(int r) {
    auto [x, y] = b[r >> 6];
...
struct wavelet_matrix {
  int n;
  vector<bit_vec> bv;
  wavelet_matrix(vector<int64_t> a, int lg): n(ssize(a)), bv(lg, {{}}) {
    for (int h = ssize(bv); h--;) {
      vector<bool> b(n);
      for (int i = 0; i < n; i++) b[i] = (~a[i] >> h) & 1;
      bv[h] = b;
      ranges::stable_partition(a, [&](int64_t x) { return (~x >> h) & 1; });
    }
  }
  int64_t kth(int l, int r, int k) {
    ...
    for (int h = ssize(bv); h--;) {
      ...
      if (k < y - x) l = x, r = y;
      else k -= y - x, res += 1LL << h, l += bv[h].cnt(n) - x, r += bv[h].cnt(n) - y;
    }
    return res;
  }
  ...
  int count(int l, int r, int64_t ub) {
    ...
    for (int h = ssize(bv); h--;) {
      int x = bv[h].cnt(l), y = bv[h].cnt(r);
      if ((ub >> h) & 1) res += y - x, l += bv[h].cnt(n) - x, r += bv[h].cnt(n) - y;
      else l = x, r = y;
    }
    return res;
  }
