...
struct PST {
  struct node {
    int64_t sum;
    int lch, rch;
    node(int64_t sum, int lch, int rch): sum(sum), lch(lch), rch(rch) {}
  };
  int root_l, root_r;
  vector<int> roots;
  deque<node> tree;
  PST(int root_l, int root_r): root_l(root_l), root_r(root_r), roots(1), tree(1, {0LL, 0, 0}) {}
  void update(int idx, int64_t change, int version) { roots.push_back(update(idx, change, root_l, root_r, roots[version])); }
  int update(int idx, int64_t change, int tl, int tr, int v) {
    if (tr - tl == 1) {
  ...
  int64_t query(int l, int r, int version) { return query(l, r, root_l, root_r, roots[version]); }
  int64_t query(int l, int r, int tl, int tr, int v) {
    if (v == 0 || r <= tl || tr <= l) return 0;
    if (l <= tl && tr <= r) return tree[v].sum;
    int tm = midpoint(tl, tr);
    return query(l, r, tl, tm, tree[v].lch) + query(l, r, tm, tr, tree[v].rch);
  }
};
//! https://cp-algorithms.com/data_structures/segment_tree.html#preserving-the-history-of-its-values-persistent-segment-tree
struct kth_smallest {
  ...
  int query(int l, int r, int k) { return query(k, pst.root_l, pst.root_r, pst.roots[l], pst.roots[r]); }
  int query(int k, int tl, int tr, int vl, int vr) {
    if (tr - tl == 1) return tl;
    int tm = midpoint(tl, tr);
    int cnt_l = pst.tree[pst.tree[vr].lch].sum - pst.tree[pst.tree[vl].lch].sum;
    if (cnt_l >= k) return query(k, tl, tm, pst.tree[vl].lch, pst.tree[vr].lch);
    return query(k - cnt_l, tm, tr, pst.tree[vl].rch, pst.tree[vr].rch);
  }
};
