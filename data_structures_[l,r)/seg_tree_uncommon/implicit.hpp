...
template<int N> struct implicit_seg_tree {
  ...
  implicit_seg_tree(int l, int r): root_l(l), root_r(r) { tree[ptr++].num = {0, r - l}; }
  ...
  void update(int l, int r, int64_t add) { update(l, r, add, root_l, root_r, 0); }
  void update(int l, int r, int64_t add, int tl, int tr, int v) {
    if (r <= tl || tr <= l) return;
    ...
    tree[v].num = op(tree[tree[v].lch].num, tree[tree[v].rch].num);
  }
  dt query(int l, int r) { return query(l, r, root_l, root_r, 0); }
  dt query(int l, int r, int tl, int tr, int v) {
    if (r <= tl || tr <= l) return unit;
    if (l <= tl && tr <= r) return tree[v].num;
    int tm = tl + (tr - tl) / 2;
    push(tl, tm, tr, v);
    return op(query(l, r, tl, tm, tree[v].lch), query(l, r, tm, tr, tree[v].rch));
  }
};
