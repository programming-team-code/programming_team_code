...
struct seg_tree {
  ...
  void update(int l, int r, int64_t d) { update(l, r, d, 0, n, 1); }
  void update(int l, int r, int64_t d, int tl, int tr, int v) {
    if (r <= tl || tr <= l) return;
  ...
  int64_t query(int l, int r) { return query(l, r, 0, n, 1); }
  int64_t query(int l, int r, int tl, int tr, int v) {
    if (r <= tl || tr <= l) return 0;
    if (l <= tl && tr <= r) return tree[v];
    int tm = split(tl, tr);
    push(tl, tm, tr, v);
    return op(query(l, r, tl, tm, 2 * v), query(l, r, tm, tr, 2 * v + 1));
  }
  //! @code
  //!   seg_tree st(n);
  ...
  int find_first(int l, int r, auto f) { return find_first_in_range(l, r, f, 0, n, 1); }
  ...
  int find_first_in_range(int l, int r, auto f, int tl, int tr, int v) {
    if (r <= tl || tr <= l) return r;
    if (l <= tl && tr <= r) return f(tree[v], tl, tr) ? find_first_in_subtree(f, tl, tr, v) : r;
    int tm = split(tl, tr);
    push(tl, tm, tr, v);
  ...
  //! @time O(log(n))
  //! @space O(log(n)) for recursion stack
  int find_last(int l, int r, auto f) { return find_last_in_range(l, r, f, 0, n, 1); }
  //! invariant: f(tree[v], tl, tr) is 1
  int find_last_in_subtree(auto f, int tl, int tr, int v) {
    if (v >= n) return tl;
    int tm = split(tl, tr);
    push(tl, tm, tr, v);
    if (f(tree[2 * v + 1], tm, tr)) return find_last_in_subtree(f, tm, tr, 2 * v + 1);
    return find_last_in_subtree(f, tl, tm, 2 * v);
  }
  int find_last_in_range(int l, int r, auto f, int tl, int tr, int v) {
    if (r <= tl || tr <= l) return l - 1;
    if (l <= tl && tr <= r) return f(tree[v], tl, tr) ? find_last_in_subtree(f, tl, tr, v) : l - 1;
    int tm = split(tl, tr);
    push(tl, tm, tr, v);
