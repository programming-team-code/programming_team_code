...
template<class T, class F> struct deq {
  ...
  void push_back(T val) { r.push_back({val, empty(r) ? val : op(r.back()[1], val)}); }
  void pop_back() {
    if (empty(r)) {
      vector<T> a(ssize(l));
      ranges::transform(l, rbegin(a), [](dt& x) { return x[0]; });
      rebuild(a, ssize(a) / 2);
    }
    r.pop_back();
  }
  void push_front(T val) { l.push_back({val, empty(l) ? val : op(val, l.back()[1])}); }
  void pop_front() {
    if (empty(l)) {
      vector<T> a(ssize(r));
      ranges::transform(r, begin(a), [](dt& x) { return x[0]; });
  ...
  void rebuild(const vector<T>& a, int sz_le) {
    ...
    ranges::transform(a | views::take(sz_le), presum, rbegin(l), [](T x, T y) { return dt{x, y}; });
    ranges::transform(a | views::drop(sz_le), presum | views::drop(sz_le), begin(r), [](T x, T y) { return dt{x, y}; });
  }
};
