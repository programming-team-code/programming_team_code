...
template<class T, class F> struct linear_rmq {
  ...
  linear_rmq(const vector<T>& a, F cmp): n(ssize(a)), a(a), cmp(cmp), in(n), asc(n), head(n + 1) {
    ...
    for (int i = 0; i < n + 1; i++) {
      int prev = 0;
      while (ssize(st) > 1 && (i == n || !cmp(a[st.back()], a[i]))) {
        head[prev] = st.back();
...
struct perm_tree {
  ...
  bool touches(int u, int v) { return p[u].mn_num == p[v].mn_num + p[v].len || p[v].mn_num == p[u].mn_num + p[u].len; }
  int allocate(int mn_i, int mn_v, int ln, bool join, const vector<int>& chs) {
  ...
  perm_tree(const vector<int>& a) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      while (!empty(st)) {
        ...
        if (touches(v, u)) {
          v = allocate(p[u].mn_idx, min(p[v].mn_num, p[u].mn_num), p[v].len + p[u].len, 1, {u, v});
          st.pop_back();
          continue;
        }
        int l = min(p[u].mn_idx, mn_i[p[v].mn_idx]), r = max(i, mx_i[p[v].mn_idx]), idx = ssize(st) - 1;
        while (r == i && l != p[st[idx][0]].mn_idx) l = min(l, st[idx][1]), r = max(r, st[idx][2]), idx = st[idx][3];
        if (r > i) {
        ...
        for (int j = idx; j < ssize(st); j++) min_num = min(min_num, p[chs[j - idx] = st[j][0]].mn_num);
