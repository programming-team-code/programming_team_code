...
template <class DS, class... ARGS> struct pq_updates {
  ...
  void pop_update() {
    vector<upd> extra;
    int idx = (int)size(upd_st) - 1, lowest_pri = INT_MAX;
    for (auto it = rbegin(mp); 2 * (int)size(extra) < (int)size(upd_st) - idx; it++) {
      auto [pri, idx_sk] = *it;
      extra.push_back(upd_st[idx_sk]);
      idx = min(idx, idx_sk), lowest_pri = pri;
    }
    auto it = remove_if(idx + begin(upd_st), end(upd_st), [&](auto& curr) {
      return curr.second->first >= lowest_pri;
    });
    reverse_copy(begin(extra), end(extra), it);
    for (int i = idx; i < ((int)size(upd_st)); i++) ds.undo();
    upd_st.pop_back();
    mp.erase(prev(end(mp)));
    for (int i = idx; i < ((int)size(upd_st)); i++) {
  ...
  void push_update(ARGS... args, int priority) {
    ds.join(args...);
    auto [it, ins] = mp.emplace(priority, (int)size(upd_st));
    assert(ins);
