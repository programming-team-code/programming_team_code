...
struct merge_sort_tree_updates {
   ...
   */
  merge_sort_tree_updates(const vector<int>& a, const vector<bool>& active) : n((int)size(a)), sorted(n), perm(n), bool_presums(n, vector<bool>()), bool_bits(2 * n, vector<bool>()) {
    if (!n) return;
  ...
  void build(const vector<int>& a, const vector<bool>& active, vector<pair<int, bool>>& cpy, int tl, int tr, int v) {
    if (tr - tl == 1) {
  ...
  int rect_count_impl(int le, int ri, int xi, int yi, int tl, int tr, int v) {
