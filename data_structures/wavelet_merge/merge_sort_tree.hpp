...
struct merge_sort_tree {
   ...
   */
  merge_sort_tree(const vector<int>& a) : n((int)size(a)), sorted(n), bool_presums(n, vector<bool>()) {
    vector<pair<int, bool>> cpy(n);
    transform(begin(a), end(a), begin(cpy), [](int num) { return pair(num, 0); });
    build(cpy, 0, n, 1);
    transform(begin(cpy), end(cpy), begin(sorted), [](auto& num) { return num.first; });
  }
