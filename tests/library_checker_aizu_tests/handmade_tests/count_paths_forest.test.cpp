...
vector<vector<ll>> naive(const vector<vector<int>>& adj, dsu_restorable& dsu) {
  tree_lift tl(adj);
  int n = sz(adj);
    ...
    for (int v = u; v < n; v++) {
      if (dsu.same_set(u, v)) {
        ...
        for (int i = 0; i <= path_length_edges; i++)
          cnts_naive[path_length_edges][tl.kth_path(u, v, i)]++;
      }
    }
  }
  return cnts_naive;
...
int main() {
  ...
  for (int n = 1; n <= 100; n++) {
    ...
    for (int k = 1; k < n; k++) {
      ...
      ll total_paths = accumulate(begin(count_paths), end(count_paths), 0LL);
