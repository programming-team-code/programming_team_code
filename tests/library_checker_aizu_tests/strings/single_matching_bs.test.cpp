...
// since this causes an O(n) partition check for
// each call to `lower_bound`, causing TLE.
...
int main() {
  ...
  sort(begin(matches), end(matches));
  {
    ...
    for (const vector<pii>& subs : tests) {
      ...
      assert(
        both.substr(s_le2, s_ri2 - s_le2) == t);
      assert(sa_ri2 - sa_le2 == 1 + sa_ri - sa_le);
      vector<int> matches_other(
        begin(lq_both.sa) + sa_le2,
        begin(lq_both.sa) + sa_ri2);
      matches_other.erase(
        remove_if(begin(matches_other),
          end(matches_other),
          [&](
            int val) { return val >= sz(s) + 1; }),
        end(matches_other));
      sort(
        begin(matches_other), end(matches_other));
