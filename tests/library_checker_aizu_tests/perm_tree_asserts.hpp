...
perm_tree perm_tree_asserts(const vector<int>& a) {
  ...
  for (int u = 0; u < sz(adj); u++) {
    {
      int mn_num_rmq = rmq_min.query(
          pt.t[u].mn_idx, pt.t[u].mn_idx + pt.t[u].len);
      assert(mn_num_rmq == pt.t[u].mn_num);
      int mx_num_rmq = rmq_max.query(
          pt.t[u].mn_idx, pt.t[u].mn_idx + pt.t[u].len);
      ...
      if (sz(adj[u]) <= 10) {
        for (int i = 0; i < sz(adj[u]); i++) {
          ...
          for (int j = i + 1; j < sz(adj[u]); j++) {
            ...
            running_max =
                max(running_max, pt.t[node_ri].mn_num +
                                     pt.t[node_ri].len);
