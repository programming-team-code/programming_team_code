  ...
  perm_tree pt(a);
  {
    ...
    while (!empty(q)) {
      ...
      assert(pt.t[u].mn_idx ==
        n -
          (pt_reverse.t[u_reverse].mn_idx +
            pt_reverse.t[u_reverse].len));
      for (int i = 0; i < sz(pt.adj[u]); i++)
        q.push({pt.adj[u][i],
          pt_reverse
            .adj[u_reverse]
                [sz(pt_reverse.adj[u_reverse]) - i - 1]});
    }
  }
    ...
    if (empty(adj[u])) {
      ...
      assert(pt.t[u].is_join ==
        (pt.t[u].mn_num == pt.t[adj[u][0]].mn_num ||
          pt.t[u].mn_num == pt.t[adj[u].back()].mn_num));
      assert(pt.t[u].is_join ==
        (pt.t[u].mn_num + pt.t[u].len ==
            pt.t[adj[u][0]].mn_num + pt.t[adj[u][0]].len ||
          pt.t[u].mn_num + pt.t[u].len ==
      ...
      for (int v : adj[u]) {
        ...
        mx_num_naive =
          max(mx_num_naive, pt.t[v].mn_num + pt.t[v].len);
