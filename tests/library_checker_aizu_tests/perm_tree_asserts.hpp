...
perm_tree perm_tree_asserts(const vector<int>& a) {
  ...
  for (int u = 0; u < sz(adj); u++) {
    ...
    if (empty(adj[u])) {
      ...
      assert(sz(adj[u]) >= 2);
      assert(pt.p[u].mn_idx == pt.p[adj[u][0]].mn_idx);
      assert(pt.p[u].mn_idx + pt.p[u].len ==
        pt.p[adj[u].back()].mn_idx +
          pt.p[adj[u].back()].len);
      assert(pt.p[u].is_join ==
        (pt.p[u].mn_num == pt.p[adj[u][0]].mn_num ||
          pt.p[u].mn_num == pt.p[adj[u].back()].mn_num));
      assert(pt.p[u].is_join ==
        (pt.p[u].mn_num + pt.p[u].len ==
            pt.p[adj[u][0]].mn_num + pt.p[adj[u][0]].len ||
          pt.p[u].mn_num + pt.p[u].len ==
            pt.p[adj[u].back()].mn_num +
              pt.p[adj[u].back()].len));
