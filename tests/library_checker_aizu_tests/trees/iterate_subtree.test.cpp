...
int main() {
  ...
  LCA lc(adj);
  {
    vector<int> cnt_small_iterated(n),
      cnt_big_iterated(n);
    for (int i = 0; i < n; i++) {
...
#include "../../../library/trees/lca_rmq/iterate_subtree.hpp"
  ...
  partial_sum(begin(cnt_big_iterated),
    end(cnt_big_iterated), begin(cnt_big_iterated));
  for (int i = 0; i < n; i++) {
    // at each merge (small to large), subtree size
    // doubles or more how many doubles until it
    // reaches n? sub_sz * 2^k ~= n 2^k ~= n /
    // sub_sz k ~= __lg(n / sub_sz)
