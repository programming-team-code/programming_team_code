...
int main() {
  ...
  edge_cd(adj,
    [&](const vector<vector<int>>& cd_adj, int cent,
      int split) -> void {
      array<vector<array<int64_t, 3>>, 2>
        all_backwards;
      ...
      auto dfs = [&](auto&& self, int u, int p,
        ...
        for (int v : cd_adj[u]) {
          ...
          array<int64_t, 2> curr_backw = {
            ...
            (b[e_id] * backwards[1] + c[e_id]) %
              mod};
          self(self, v, u, curr_forw, curr_backw,
            side);
        }
      };
      for (int i = 0; i < sz(cd_adj[cent]); i++) {
        int e_id = edge_id(cent, cd_adj[cent][i]);
        dfs(dfs, cd_adj[cent][i], cent,
          {b[e_id], c[e_id]}, {b[e_id], c[e_id]},
          i < split);
      }
      for (int side = 0; side < 2; side++) {
        for (auto [u, curr_b, curr_c] :
          all_backwards[side]) {
